...
public class lifeManager : MonoBehaviour
{
            ...
            for (int y = 0; y < sy; y++)
            {
                ...
                if (worldCells[x, y].type == Cell.Type.GOL) //check if neighbors are GOL!!!
                {
                    ...
                    if (worldCells[x, y].alive == 0)
                    {
                        ...
                        if (count == 2 || count == 3 || count == 4 || count == 6 && worldCells[x, y].type == Cell.Type.GOL)
                    ...
                    else if (worldCells[x, y].alive == 1)
                    {
                        ...
                        { //am alive, how many neighbors trigger me dying
                            ...
                            //worldCellsNext[x, y].type = Cell.Type.sand; //maze maker!
                ...
                else if (worldCells[x, y].type == Cell.Type.sand)
                {
                    ...
                    if (worldCells[(x + sx - 1) % sx, (y + sy - 1) % sy].alive == 0 && worldCells[(x + 1) % sx, (y + sy - 1) % sy].alive == 0) // down
                    {
                        //check if something can be born
                        if (count == 1 || count == 2 || count == 3 && worldCells[x, y].type == Cell.Type.sand)
                        { //am dead, how many neighbors trigger a birth
                            worldCellsNext[x, y].alive = 1; //come alive
                            worldCellsNext[x, y].type = Cell.Type.sand;

                            //worldCellsNext[x, y - 1].alive = 1;
                            //worldCellsNext[x, y - 1].type = Cell.Type.sand;
                        }
                        else
                        {
                            //am dead and stayed dead
                    ...
                    else if (worldCells[(x + 1) % sx, (y + 1) % sy].alive == 1 && worldCells[(x + sx - 1) % sx, (y + 1) % sy].alive == 1) //kill go up?
                    {
                        //check if the cell should die
                        ...
                        else
                        {
                            ...
                            if (worldCellsNext[x, y].age >= 10)
                            {
                                //worldCellsNext[x, y].alive = 0; //die //ant hill
                                //worldCellsNext[x, y].age = 0;
                            }
                        }
                ...
                else if (worldCells[x, y].type == Cell.Type.ant_hill)
                {
                    //do sand logic
    ...
    int neighbors(int x, int y, Cell.Type _type)
    {
        //this wraps around the edges
        int toreturn = 0;
        //check & care about types
        if (worldCells[(x + 1) % sx, y].type == _type) toreturn += worldCells[(x + 1) % sx, y].alive;
        //worldCells[(x + 1) % sx, y].alive +
        //worldCells[x, (y + 1) % sy].alive +
        //worldCells[(x + sx - 1) % sx, y].alive +
        //worldCells[x, (y + sy - 1) % sy].alive + //
        //worldCells[(x + 1) % sx, (y + 1) % sy].alive + //move top right
    ...
    public class Cell
    {
        public enum Type { GOL, sand, ant_hill,}
