...
public class lifeDrawer : MonoBehaviour
{
    ...
    public Gradient sandgradient;
    public Gradient antgradient;
    ...
    public void DrawLife(lifeManager.Cell[,] cells)
    {
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                if (cells[x, y].alive == 1 && cells[x, y].type == lifeManager.Cell.Type.sand)
                {
                    //array_Main[x + (y * width)] = Color.Lerp(aliveCol, aliveCol2, Mathf.PingPong(Time.time, 1));
                    float agepercent = (float)cells[x, y].age / 10f;
                    array_Main[x + (y * width)] = sandgradient.Evaluate(agepercent);
                }
                else if (cells[x, y].alive == 0)
                {
                    array_Main[x + (y * width)] = deadCol; //black
                ...
                if (cells[x, y].alive == 1 && cells[x, y].type == lifeManager.Cell.Type.ant_hill)
                {
                    //array_Main[x + (y * width)] = Color.Lerp(aliveCol, aliveCol2, Mathf.PingPong(Time.time, 1));
                    float agepercent = (float)cells[x, y].age / 10f;
                    array_Main[x + (y * width)] = antgradient.Evaluate(agepercent);
                }
                else if (cells[x, y].alive == 0)
                {
                    array_Main[x + (y * width)] = deadCol; //black
